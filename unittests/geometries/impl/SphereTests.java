...
class SphereTests {
    ...
    /** Radius shared by both spheres in this class. */
    private static final double RADIUS_1 = 1d;

    // ============ Fields for testGetNormal ==============
    /** Sphere center for the getNormal test. */
    private static final Point CENTER = new Point(1, 1, 1);
    /** Point on sphere surface, +Y from center. Used in: EP01. */
    private static final Point SURFACE_POINT = new Point(1, 2, 1);
    /** Expected normal at SURFACE_POINT. Used in: EP01. */
    private static final Vector EXPECTED_NORMAL = new Vector(0, 1, 0);
    /** Sphere under test. Used in: testGetNormal. */
    private static final Sphere SPHERE_1 = new Sphere(CENTER, RADIUS_1);

    // ============ Fields for testFindIntersections ==============
    /** Center of the sphere for all intersection cases (sphere spans x = [0, 2]). */
    private static final Point CENTER_INT = new Point(1, 0, 0);
    /** Sphere under test. Used in: testFindIntersections. */
    private static final Sphere SPHERE_INT = new Sphere(CENTER_INT, RADIUS_1);

    /** Ray origin outside the sphere on -X axis. Used in: EP01, EP02, BV03. */
    private static final Point P1 = new Point(-1, 0, 0);
    /** Ray origin inside the sphere. Used in: EP03, BV05, BV13. */
    private static final Point P2 = new Point(1, 0.5, 0);
    /** Ray origin beyond the sphere on +X axis. Used in: EP04, BV08, BV12. */
    private static final Point P3 = new Point(3, 0, 0);
    /** Near intersection point, -X side of sphere. Used in: EP02, BV01, BV02, BV04, BV06. */
    private static final Point P4 = new Point(0, 0, 0);
    /** Ray origin at the sphere's center. Used in: BV06. */
    private static final Point P5 = new Point(1, 0, 0);
    /** Ray origin before the tangent point. Used in: BV09. */
    private static final Point P6 = new Point(0, 1, -1);
    /** Ray origin at the tangent point. Used in: BV10. */
    private static final Point P7 = new Point(0, 1, 0);
    /** Ray origin after the tangent point. Used in: BV11. */
    private static final Point P8 = new Point(0, 1, 1);
    /** Far intersection point, +X side of sphere. Used in: EP02, BV01, BV06, BV07. */
    private static final Point P9 = new Point(2, 0, 0);
    /** Exit point for a ray starting inside the sphere. Used in: EP03. */
    private static final Point P10 = new Point(1, 1, 0);

    /** Direction missing the sphere entirely. Used in: EP01. */
    private static final Vector V1 = new Vector(1, 1, 0);
    /** Non-unit direction along +X. Used in: EP02. */
    private static final Vector V2 = new Vector(3, 0, 0);
    /** Direction along +Y. Used in: EP03, BV12. */
    private static final Vector V3 = new Vector(0, 1, 0);
    /** Unit direction along +X. Used in: EP04, BV01, BV03-BV08. */
    private static final Vector V4 = new Vector(1, 0, 0);
    /** Direction along -X. Used in: BV02. */
    private static final Vector V5 = new Vector(-1, 0, 0);
    /** Direction along +Z. Used in: BV09-BV11, BV13. */
    ...
    void testGetNormal() {
        ...
        Vector normal = SPHERE_1.getNormal(SURFACE_POINT);
    ...
    void testFindIntersections() {
        ...
        assertEquals(List.of(P4, P9), resultEP02, ERROR_RESULT);
        ...
        assertEquals(List.of(P10), resultEP03, ERROR_RESULT);
        ...
        List<Point> resultBV01 = SPHERE_INT.findIntersections(new Ray(P4, V4));
        assertNotNull(resultBV01, ERROR_RESULT);
        assertEquals(1, resultBV01.size(), ERROR_WRONG_NUM_OF_INTERSECTIONS);
        assertEquals(List.of(P9), resultBV01, ERROR_RESULT);

        // BV02: Ray starts at sphere and goes outside (0 points)
        assertNull(SPHERE_INT.findIntersections(new Ray(P4, V5)), ERROR_NO_INTERSECTION);
        ...
        List<Point> resultBV04 = SPHERE_INT.findIntersections(new Ray(P4, V4));
        ...
        List<Point> resultBV06 = SPHERE_INT.findIntersections(new Ray(P5, V4));
        assertNotNull(resultBV06, ERROR_RESULT);
        assertEquals(1, resultBV06.size(), ERROR_WRONG_NUM_OF_INTERSECTIONS);
        assertEquals(List.of(P9), resultBV06, ERROR_RESULT);

        // BV07: Ray starts at sphere and goes outside (0 points)
        assertNull(SPHERE_INT.findIntersections(new Ray(P9, V4)), ERROR_NO_INTERSECTION);
        ...
        assertNull(SPHERE_INT.findIntersections(new Ray(P6, V6)), ERROR_NO_INTERSECTION);
